...
for (i in namelist){
  assign(i,
         arrange(ddply(plotCont[plotCont$names==i,],.(names),
                       summarize,count=count(contbr_city))$count,desc(freq)))
  i<-get(i)
  variable_list<-c(variable_list,i)
  print(head(i,n = 10))
  }
```
...
for (i in namelist){a<-get(i)
                 a$lat<- lapply(as.character(a$x),
                                        function(x)
                                          if (!is.na(x)){
                                            geocode(x,source ="dsk",messaging = FALSE)$lat}
                                        else {NA})
                 a$lon<- lapply(as.character(a$x),
                                       function(x) 
                                         if (!is.na(x)){
...
for (i in namelist){
  ggplot()+borders("world",colour="gray50",fill="gray50")+
    geom_point(data=get(i),mapping=aes(x=unlist(lon),unlist(lat),col='blue'))
}
```